...
class Converter():
    ...
    def read_folder_contents(self, folder):
        ...
        for fileName in folderContents:                                         # this loop does the sorting of .con and .jpg files
            ...
            if ".jpg" in fileName:
                ...
                pass
        self.jpgPaths = sorted(self.jpgPaths)                                   # sort the .jpg paths because the .con file columns 
    ...
    def read_con_file(self):
        ...
        conFile.close()
        self.numFiles = ((len(conLines[0].strip().split())) / 2)                # count number of columns in file and divide by 2 
                                                                                # (since 2 columns to each image file)
        self.splitCoords = [[] for i in range(self.numFiles)]                   # create list to append paired coordinates for 
                                                                                # each image file
    ...
    def print_new_files(self):
        ...
        for fileNum in range(self.numFiles):
            outFile= open(self.folder + '/output_' +
                          str(self.jpgPaths[fileNum]) + '.txt' , 'w')
            i=0
