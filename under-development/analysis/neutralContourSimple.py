...
def plotTvB(contours, backNum, tipNum, neutral='neutral.csv'):
	'''backNum is the point on the contour from which to extract back peak
	   value similarly, tipNum tells where to extract tip peak value
	'''
	t = NeutralTongue()
	cx, cy = t.getNeutral(neutral)
	cmags = t.makePolar(cx, cy)
...
def analyze(files, results):
	...
	for i in range(len(f)):
		...
		o.write("%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n" % (contours[:-4],
                        lag,backNum,tipNum,lstart,lend,indb,indt))
